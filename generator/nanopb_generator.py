...
class ProtoFile:
    ...
    def generate_cppheader(self, includes, headername, cppheadername, options):
        ...
        if self.fdesc.package:
            symbol = make_identifier(self.fdesc.package + '_' + cppheadername)
        else:
            symbol = make_identifier(cppheadername)
        yield '#ifndef PB_%s_INCLUDED\n' % symbol
        yield '#define PB_%s_INCLUDED\n' % symbol
        try:
        ...
        for incfile in includes:
            noext = os.path.splitext(incfile)[0]
            yield options.genformat % (noext + options.extension + '.hpp')
            yield '\n'
...
def process_file(filename, fdesc, options, other_files = {}):
    ...
    Returns a dict:
        {'headername': Name of header file,
         ...
         'cppheadername': Name of C++ header file,
         'cppheaderdata': Data for the .hpp header file
        }
    '''
    ...
    cppheadername = noext + options.extension + '.hpp'
    headerbasename = os.path.basename(headername)
    cppheaderbasename = os.path.basename(cppheadername)
    ...
    cppheaderdata = ''.join(f.generate_cppheader(includes, headerbasename, cppheaderbasename, options))
    ...
    return {'headername': headername, 'headerdata': headerdata,
            ...
            'cppheadername': cppheadername, 'cppheaderdata': cppheaderdata}
...
def main_cli():
    ...
    for filename in filenames:
        ...
        to_write = [
            ...
            (os.path.join(base_dir, results['cppheadername']), results['cppheaderdata']),
        ]
    ...
    for filename in request.file_to_generate:
        for fdesc in request.proto_file:
            if fdesc.name == filename:
                ...
                f = response.file.add()
                f.name = results['cppheadername']
                f.content = results['cppheaderdata']
