...
class ProtoFile:
    ...
    def generate_cxxheader(self, includes, headername, cxxheadername, options):
        '''Generate content for a C++ header file.
        Generates strings, which should be concatenated and stored to file.
        '''
        ...
        if self.fdesc.package:
            symbol = make_identifier(self.fdesc.package + '_' + cxxheadername)
        else:
            symbol = make_identifier(cxxheadername)
        yield '#ifndef PB_%s_INCLUDED\n' % symbol
        yield '#define PB_%s_INCLUDED\n' % symbol
        try:
            yield options.libformat % ('pb.hxx')
        except TypeError:
            # no %s specified - use whatever was passed in as options.libformat
            yield options.libformat
        yield '\n'
        ...
        for incfile in includes:
            noext = os.path.splitext(incfile)[0]
            yield options.genformat % (noext + options.extension + '.hxx')
            yield '\n'
...
def process_file(filename, fdesc, options, other_files = {}):
    ...
    Returns a dict:
        {'headername': Name of header file,
         ...
         'cxxheadername': Name of C++ header file,
         'cxxheaderdata': Data for the .hxx header file
        }
    '''
    ...
    cxxheadername = noext + options.extension + '.hxx'
    headerbasename = os.path.basename(headername)
    cxxheaderbasename = os.path.basename(cxxheadername)
    ...
    cxxheaderdata = ''.join(f.generate_cxxheader(includes, headerbasename, cxxheaderbasename, options))
    ...
    return {'headername': headername, 'headerdata': headerdata,
            ...
            'cxxheadername': cxxheadername, 'cxxheaderdata': cxxheaderdata}
...
def main_cli():
    ...
    for filename in filenames:
        ...
        to_write = [
            ...
            (os.path.join(base_dir, results['cxxheadername']), results['cxxheaderdata']),
        ]
    ...
    for filename in request.file_to_generate:
        for fdesc in request.proto_file:
            if fdesc.name == filename:
                ...
                f = response.file.add()
                f.name = results['cxxheadername']
                f.content = results['cxxheaderdata']
